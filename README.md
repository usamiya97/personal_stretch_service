...
## 運用・保守・セキュリティ
* **APIエンドポイントの保護:** エントリーポイントにAPI Gatewayを配置し、不正なアクセスやDDoS攻撃をエッジ側で緩和。
* **認証・認可:** JWTを用いたステートレスな認証基盤を実装。Spring Securityにより、バックエンド（ECS）到達時の緻密な認可制御を実施。
* **Public Subnet配置におけるアクセス制御（トレードオフの緩和）:**
  コスト最適化（NAT Gateway/ALBの排除）のためECSをPublic Subnetに配置していますが、AWSのセキュリティグループによるインバウンド通信の厳格な制限と、アプリケーションレイヤー（Spring Boot）でのJWTトークン検証により、多層防御を実現しています。

### 運用 (Operations)
* **フルマネージド＆サーバーレスの活用:** Vercel、API Gateway、Lambda、Supabaseといったサーバーレス/マネージドサービスを組み合わせることで、OSのパッチ当て等の運用保守作業を排除し、ゼロダウンタイムでの運用を可能にしています。
* **コスト監視:** AWS Budgetsを利用し、万が一のトラフィック急増時にも意図しない高額課金を防ぐアラート体制を構築しています。

### 保守 (Maintenance)
* **コンテナ化によるポータビリティの確保:**
  バックエンド（Spring Boot）をDockerコンテナ化してECSで稼働させています。これにより、将来的に予算が確保でき、「理想の構成（Private Subnet + ALB）」へ移行する際も、アプリケーションコードを変更することなくスムーズにインフラ構成のスケールアップが可能です。
* **フロントエンドとバックエンドの完全分離:**
  VercelとAWSでリポジトリ・インフラを完全に分離（疎結合）しているため、機能拡張や改修時の影響範囲を極小化し、独立したデプロイサイクルを回せるようにしています。
