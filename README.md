...
## 運用・保守・セキュリティ
* **セキュリティ**
  * **認証・認可:** JWTとSpring Securityを組み合わせたステートレスなAPI認証を実装。
  * **ネットワーク保護:** API GatewayとLambdaの連携により、バックエンドの主要リソースを直接パブリックに露出させない構成を採用。
* **運用（監視・CI/CD）**
  * **コスト監視:** AWS Budgetsを利用し、意図しないリソース消費時のアラートを事前設定。コストゼロ運用の継続性を担保。
  * **自動デプロイ:** GitHub Actionsを利用し、mainブランチへのマージをトリガーとしたVercel（フロント）およびAWS（バックエンド）へのデプロイパイプラインを構築。
* **保守性（IaC・将来への備え）**
  * **インフラのコード化:** Terraformを用いてインフラをコード管理。現在はコスト最適化構成（Supabase/Lambda）で稼働中ですが、将来的なトラフィック増加に合わせて、シームレスに「理想の構成（RDS/ECS）」へ移行できる保守性を確保しています。
